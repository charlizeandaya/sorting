...
def _merged(xs, ys, cmp=cmp_standard):
    ...
    for i in range(total_len):
        ...
        if compare <= 0:
            merge_list.append(xs[x])
            if x < len(xs) - 1:
                x += 1
            else:
                return merge_list + ys[y:]
        else:
            merge_list.append(ys[y])
            if y < len(ys) - 1:
                y += 1
            else:
                return merge_list + xs[x:]
...
def merge_sorted(xs, cmp=cmp_standard):
    ...
    if len(xs) <= 1:
        ...
        mid = len(xs) // 2
        left_list_sorted = merge_sorted(xs[:mid], cmp)
        right_list_sorted = merge_sorted(xs[mid:], cmp)
        merged_list = _merged(left_list_sorted, right_list_sorted, cmp)
        return merged_list
...
def quick_sorted(xs, cmp=cmp_standard):
    ...
    if len(xs) <= 1:
        ...
        pivot_val = random.choice(xs)

        less_than_xs = [val for val in xs if cmp(val, pivot_val) < 0]
        greater_than_xs = [val for val in xs if cmp(val, pivot_val) > 0]
        equal_list = [val for val in xs if cmp(val, pivot_val) == 0]
        less_than_xs = quick_sorted(less_than_xs, cmp)
        greater_than_xs = quick_sorted(greater_than_xs, cmp)

        return less_than_xs + equal_list + greater_than_xs
...
def quick_sort(xs, cmp=cmp_standard):
    ...
    copy of the list.
    '''
    _helper(xs, 0, len(xs) - 1, cmp)


def _helper(xs, lo, hi, cmp):
    if lo < hi:
        p = partition(xs, lo, hi, cmp)
        _helper(xs, lo, p - 1, cmp)
        _helper(xs, p + 1, hi, cmp)
